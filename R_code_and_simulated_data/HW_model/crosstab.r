...
pop.size <- c()
for (i in 1:length(unique(pop.id)))
{

pop.size <- c(pop.size,count(unique(pop.id)[i],pop.id))
}
...
rownames(popu) <- c(unique(pop.id))


setwd('Chapter2/R_code_and_simulated_data/Data')
write.xlsx(data.frame(popu), "cross_tab_region.xlsx", sheetName="Sheet1")
